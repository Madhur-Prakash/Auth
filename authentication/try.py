...
async def cache(data: str, plain_password):
    ...
    if CachedData and user:
        ...
        hashed_password = await Hash.verify(user["password"], plain_password)
        if hashed_password:
            print("searching inside db")
            await client.set(f"patient:{data}",data, ex=30)
            logger.info(f"Patient logged in successfully using: {data}")
            return data
    return None
...
async def signup(request: Request):
    try:
        ...
        regex_username_pattern = r"^[a-zA-Z0-9_.-]{3,}$"
        ...
        # Changed from 409 to 400 for validation errors
...
@auth_patient.post("/patient/login", status_code=status.HTTP_200_OK)  # Changed from 202 to 200
    ...
    try:
        ...
        if not email_provided and not user_name_provided and not phone_number_provided:
            ...
            if email_provided:
                ...
                if cached_data:
                    print("cache data returned", cached_data)
                    access_token = auth_token.create_access_token(data={"sub": cache_key})
                    response = RedirectResponse("http://127.0.0.1:8000", status_code=status.HTTP_200_OK)
                    response.delete_cookie("access_token")
                    response.set_cookie(key="access_token", value=access_token, max_age=3600)
                ...
                print("cache data returned none")
                if not user:
                ...
                if cached_data:
                    print("cache data returned", cached_data)
                    access_token = auth_token.create_access_token(data={"sub": cache_key})
                    response = RedirectResponse("http://127.0.0.1:8000", status_code=status.HTTP_200_OK)  # Changed from 201 to 200
                    response.delete_cookie("access_token")
                    response.set_cookie(key="access_token", value=access_token, max_age=3600)
                    return response
...
async def logout(patient_user_name: str):
    ...
    print(f"{patient_user_name} logged out successfully")
    return response