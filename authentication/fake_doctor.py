...
# Connect to MongoDB
client = MongoClient("mongodb://localhost:27017/auth")  # Change this if your MongoDB is hosted elsewhere

# Function to generate a fake user
def generate_fake_user():
    full_name = fake.name()
    email = fake.email()
    doctor__user_name = fake.user_name()  # Use a valid Faker method for generating usernames
    ...
    return {
        "full_name": full_name,
        "email": email,
        "doctor_user_name": doctor__user_name,
...
client.auth.doctor.insert_many(fake_users)
