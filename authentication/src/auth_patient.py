...
async def signup(data: models.patient, response: Response, request: Request):
    try:
        ...
        return {"message":f"Account for patient created successfully: {dict_data['email']}", "status_code":status.HTTP_201_CREATED, "token_type":"Bearer"}
        ...
        <tr>
            ...
            # login using phone_number and password
                ...
                if cached_data:
                    ...
                    return {"message": f"OTP sent successfully on {phone_number_provided[3:106]+'x'*6+phone_number_provided[13:]}", "status_code":status.HTTP_200_OK} # masking the phone number for security
...
async def refresh_token(request: Request, response: Response):
    try:
        ...
        if verify_refresh_token and verify_device_fingerprint and verify_session_id:  # if refresh token is valid, give access token
            ...
            return({"status_code":status.HTTP_200_OK, "message":"Refresh token verified,patient logged in", "token_type":"Bearer"})
