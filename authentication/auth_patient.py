...
@auth_patient.post("/patient/{email}/verify_otp_signup", status_code=status.HTTP_200_OK, response_class=HTMLResponse) # verify otp
...
@auth_patient.post("/patient/{phone_number}/verify_otp_signup", status_code=status.HTTP_200_OK, response_class=HTMLResponse) # verify otp
async def verify_otp_signup(request: Request, phone_number: str, response: Response):
    try:
        ...
        user = await mongo_client.auth.patient.find_one({"phone_number": otp_stored.get("phone_number")})
        if user:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="User already exists")
        # Insert into MongoDB
        await mongo_client.auth.patient.insert_one(mongodb_document)
        ...
        logger.info(f"Account for patient created successfully: {phone_number}")
...
@auth_patient.post("/patient/login_otp", status_code=status.HTTP_200_OK, response_class=HTMLResponse) # login using email 
    ...
    try:
        ...
        if not email_provided and not phone_number_provided:
            ...
            if email_provided:
                ...
                logger.warning(f"login attempt with invalid Invalid credentials: {form_data['email']}")
                raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid credentials")
...
@auth_patient.post("/patient/{email}/verify_otp_login", status_code=status.HTTP_200_OK, response_class=HTMLResponse) 
...
@auth_patient.post("/patient/{phone_number}/verify_otp_login", status_code=status.HTTP_200_OK, response_class=HTMLResponse)
